...
def run_config(name, config_file, tday = datetime.date.today(), agent_class = 'agent.Agent'):
    base.config_logging("ctp_" + name + ".log", level=logging.DEBUG,
                   format = '%(name)s:%(funcName)s:%(lineno)d:%(asctime)s %(levelname)s %(message)s',
                   to_console = True,
                   console_level = logging.INFO)
    myApp = MainApp(name, tday, config_file, agent_class = agent_class, master = None)
    myGui = Gui(myApp)
    # myGui.iconbitmap(r'c:\Python27\DLLs\thumbs-up-emoticon.ico')
    myGui.mainloop()
...
if __name__ == '__main__':
    ...
    if len(args) < 1:
        config_file = 'prod_test_agent.json'
    else:
        config_file = args[0]
    params = (app_name, config_file, tday, agent_class, )
    run_config(*params)
