...
class TraderMock(agent.CTPTraderQryMixin):
    ...
    def __init__(self,myagent):
        ...
        self.session_id = 0
        self.agent = myagent
        self.eventEngine = myagent.eventEngine
    ...
    def ReqOrderInsert(self, order, request_id):
        ...
        # 特定合约成交事件
        event2 = Event(type=(EVENT_TRADE_CONTRACT+trade.InstrumentID))
    ...
    def ReqOrderAction(self, corder, request_id):
        ...
        event2.dict['data'] = rorder
        event2.dict['error'] = BaseObject( ErrorID = 0, ErrorMsg = "cancel order")
        self.eventEngine.put(event2)          
    ...
    def ReqQryTradingAccount(self,req,req_id=0):
        ...
        event.dict['data'] = BaseObject( Available = self.available)
        event.dict['error'] = BaseObject( ErrorID = 0, ErrorMsg = "trading account")
    ...
    def ReqQryInstrument(self,req,req_id=0):#只有唯一一个合约
        ...
        pinst = BaseObject(InstrumentID = instID, 
                           VolumeMultiple = inst.multiple, 
                           PriceTick = inst.tick_base, 
                           LongMarginRatio = inst.marginrate[0], 
                           ShortMarginRatio= inst.marginrate[1])
        event = Event(type = EVENT_INSTRUMENT)
        event.dict['data'] = pinst
        event.dict['error'] = BaseObject( ErrorID = 0, ErrorMsg = "query instrument")
        self.eventEngine.put(event)          
    ...
    def ReqQryInstrumentMarginRate(self,req,req_id=0):
        ...
        event.dict['data'] = pinst
        event.dict['error'] = BaseObject( ErrorID = 0, ErrorMsg = "query margin rate")
        self.eventEngine.put(event)          
