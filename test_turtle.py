...
def turtle_sim( assets, start_date, end_date ):
    ...
    results = {}
    for pc in assets:
        ddata[pc] = misc.rolling_hist_data(pc, 1, start_date, end_date, '-20b', 'd', '-55b')
		mdata[pc] = misc.rolling_hist_data(pc, 1, start_date, end_date, '-20b', 'm', '-1b')
        res = {}
        for i in range(len(ddata[pc])):
            ...
            df = ddf.join(ts)
            ddf['OL_1'] = pd.concat([df.DONCH_H20.shift(1), df.open], join='outer', axis=1).max(axis=1)
            ddf['OS_1'] = pd.concat([df.DONCH_L20.shift(1), df.open], join='outer', axis=1).min(axis=1)
            ddf['CL_1'] = pd.concat([df.DONCH_L10.shift(1), df.open], join='outer', axis=1).min(axis=1)
            ddf['CS_1'] = pd.concat([df.DONCH_H10.shift(1), df.open], join='outer', axis=1).max(axis=1)
            ...
            for idx, dd in enumerate(mdf.index):
                ...
                if len(curr_pos) == 0 and idx < len(df.index)-1:
                    ...
                    if u.close > u.OL_1:
                        ...
                        direction = -1
                        entry = u.OS_1
                    df.ix[dd, 'new_pos'] = n_unit * direction
                    if entry > 0:
                    ...
                    if (direction == 1 and u.close < u.CL_1) or (direction == -1 and u.close > u.CS_1) or (idx == len(df.index)-1):
                        ...
                        df.ix[dd, 'pnl'] = sum([trade.profit for trade in curr_pos])
                        curr_pos = []
                    #stop loss position partially
                        ...
                        for trade in curr_pos:
                            if (trade.exit - u.close) * direction > 0:
                                trade.close(u.close, dd)
                                closed_trades.append(trade)
                        df.ix[dd, 'pnl'] = sum([trade.profit for trade in curr_pos if trade.is_closed])    
                    ...
                    #add positions
                        ...
                        curr_pos.append(trade)
                df.ix[dd, 'pos'] = sum( [trade.pos for trade in curr_pos] )    
...
def turtle_sim2( assets, start_date, end_date ):
    NN = 2
    start_idx = 20
    data = {}
    results = {}
...
if __name__=="__main__":
	res = turtle_sim( ['m'], datetime.date(2014,1,2), datetime.date(2014,7,28) )
